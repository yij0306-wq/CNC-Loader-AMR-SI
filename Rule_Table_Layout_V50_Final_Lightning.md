...
* 사용자가 최대 `300배속`으로 시뮬레이션을 돌리더라도, **👀 회피 관찰 1배속 (Evasion Watch)** 기능을 켜두면, 중앙 통로에서 2대 이상의 AMR이 반경 300px 이내로 근접하는 순간 시뮬레이션 물리 연산이 강제로 **1배속**으로 떨어집니다.
* 이를 통해 마주오는 AMR이 서로 회피 존(Siding)으로 대피하는 정교한 알고리즘 동작을 슬로우 모션처럼 정밀하게 관찰할 수 있으며, 서로 멀어지면 즉시 원래의 고배속으로 복귀하여 시뮬레이션 시간을 크게 단축시킵니다.
